...
namespace GoogleARCore.Examples.HelloAR
{
    ...
    public class HelloARController : MonoBehaviour
    {
            ...
            if (Frame.Raycast(touch.position.x, touch.position.y, raycastFilter, out hit))
            {
                ...
                else
                {
                    ...
                    using (var image = Frame.CameraImage.AcquireCameraImageBytes())
                    {
                        ...
                        //_OnImageAvailable2(image.Width, image.Height, image.Y, 0);  ----
                    }
                }
            }
        }
        ...
        private void _OnImageAvailable(int width, int height, int rowStride, IntPtr pixelBuffer, int bufferSize)
        {
            ...
            Texture2D m_TextureRender = new Texture2D(width, height, TextureFormat.RGB565, false, false);
            ...
            Debug.Log(path + "/YUV2RGB.png");
            t.text = path + "/YUV2RGB.png";
        }
        ...
        private void _OnImageAvailable2(int width, int height, IntPtr pixelBuffer, int bufferSize)
        {
            //m_CachedTextureReader.enabled = false;
            //m_CachedTextureReader.OnImageAvailableCallback -= _OnImageAvailable;

            Texture2D m_EdgeDetectionBackgroundTexture = new Texture2D(width, height, TextureFormat.RGBA32, false, false);
            byte[] m_EdgeDetectionResultImage = new byte[width * height * 4];
